"""
Model performance evaluation script for AlexNet.
...
output_dir = os.path.join("results", "alexnet")
os.makedirs(output_dir, exist_ok=True)

# Data transforms (same normalization as training/other pretrained models)
...
def plot_confidence_distribution(all_probs, all_labels):
    ...
    """
    _ = all_labels  # Kept for interface consistency with other analysis helpers.
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'confidence_distribution_alexnet_final.png')
...
def plot_cm(all_labels, all_preds, class_names):
    ...
    plt.ylabel('Actual')
    plt.title('AlexNet - Songket Motif Confusion Matrix')
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'confusion_matrix_alexnet_final.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_per_class_metrics(all_labels, all_preds, class_names):
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'per_class_metrics_alexnet_final.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_accuracy_distribution(all_labels, all_preds, class_names):
    ...
    with plt.style.context('seaborn-v0_8-whitegrid'):
        ...
        # Left panel: donut chart for overall model performance.
        ...
        # Right panel: lollipop chart for per-class accuracy.
        ...
        fig.suptitle('AlexNet Performance Dashboard', fontsize=17, fontweight='bold', color='#3f2f25', y=1.02)
        plt.tight_layout()
        output_path = os.path.join(output_dir, 'accuracy_dashboard_alexnet_final.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def generate_performance_summary(all_labels, all_preds, class_names):
    ...
    print("\n" + "=" * 70)
    print(" " * 15 + "PERFORMANCE SUMMARY - AlexNet")
    print("=" * 70)
...
if __name__ == "__main__":
    MODEL_FILE = "songket_motif_alexnet_final.pth"
