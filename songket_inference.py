...
def _build_googlenet(num_classes: int, has_aux: bool = True) -> nn.Module:
    m = models.googlenet(weights=None, aux_logits=has_aux)
    m.fc = nn.Linear(m.fc.in_features, num_classes)
    if has_aux:
...
def load_model(
    ...
    if model_id == "googlenet":
        # Auto-detect whether checkpoint was saved with auxiliary heads
        has_aux = any(k.startswith("aux1") or k.startswith("aux2") for k in state.keys())
        model = _build_googlenet(NUM_CLASSES, has_aux=has_aux)
    elif model_id in _BUILDERS:
        model = _BUILDERS[model_id](NUM_CLASSES)
    else:
        raise ValueError(f"No builder registered for model ID '{model_id}'.")

    # strict=True — any key mismatch raises an error immediately so you
    # know exactly which model/checkpoint is mismatched, rather than
    # silently loading wrong weights.
...
def load_all_models(
    ...
    all_ids = list(_BUILDERS.keys()) + ["googlenet"]
...
def classify_image(
    ...
    with torch.no_grad():
        out = model(batch)
        # GoogLeNet with aux_logits=True returns a named-tuple at eval too
        # in some torchvision versions — always extract main logits safely.
