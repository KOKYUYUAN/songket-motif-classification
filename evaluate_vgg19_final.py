"""
Model performance evaluation script for VGG19.
...
output_dir = os.path.join("results", "vgg19")
...
def plot_confidence_distribution(all_probs, all_labels):
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'confidence_distribution_vgg19_final.png')
...
def plot_cm(all_labels, all_preds, class_names):
    ...
    plt.ylabel('Actual')
    plt.title('VGG19 - Songket Motif Confusion Matrix')
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'confusion_matrix_vgg19_final.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_per_class_metrics(all_labels, all_preds, class_names):
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'per_class_metrics_vgg19_final.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_accuracy_distribution(all_labels, all_preds, class_names):
    ...
    with plt.style.context('seaborn-v0_8-whitegrid'):
        ...
        fig.suptitle('VGG19 Performance Dashboard', fontsize=17, fontweight='bold', color='#3f2f25', y=1.02)
        plt.tight_layout()
        output_path = os.path.join(output_dir, 'accuracy_dashboard_vgg19_final.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def generate_performance_summary(all_labels, all_preds, class_names):
    ...
    print("\n" + "="*70)
    print(" "*15 + "PERFORMANCE SUMMARY - VGG19")
    print("="*70)
